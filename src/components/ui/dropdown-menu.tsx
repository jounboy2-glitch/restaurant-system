...
export const DropdownMenuTrigger=({asChild,children}:{asChild?:boolean;children:React.ReactElement})=>{const {setOpen}=React.useContext(Ctx);return asChild?React.cloneElement(children as any,{onClick:(e:any)=>{children.props.onClick?.(e);setOpen(true)}}):<button onClick={()=>setOpen(true)}>{children}</button>}
