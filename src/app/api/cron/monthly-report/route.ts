...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [payments, expenses, orders] = await Promise.all([
      ...
      supabase.from('expenses').select('amount').gte('created_at', monthAgo.toISOString()),
      supabase.from('orders').select('id').gte('created_at', monthAgo.toISOString()),
    ])
    ...
    const message = formatReportMessage('📊 የወሩ ሪፖርት', reportData)
